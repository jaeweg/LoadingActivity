...
public class WheelView extends View {
	...
	private static final int VALUE_TEXT_COLOR = 0xF020A591;

	/** Items text color */
	private static final int ITEMS_TEXT_COLOR = 0x4C646464;
	private static final int ITEMS_TEXT_COLOR_SENCOND = 0x99646464;
	...
	private TextPaint itemsSencondPaint;
	...
	private StaticLayout itemSencondLayout;
	...
	private float valuePaintWidth;
	private float lablePaintWidth;
	/**
	 * Constructor
	 */
	...
	private void initResourcesIfNecessary() {
		if (itemsPaint == null) {
			...
			if(mTextView != null)itemsPaint.setTypeface(mTextView.getTypeface());
		}

		if (itemsSencondPaint == null) {
			itemsSencondPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG
					/*| Paint.FAKE_BOLD_TEXT_FLAG*/);
			itemsSencondPaint.setTextSize(TEXT_SIZE);
			itemsSencondPaint.setShadowLayer(0.1f, 0, 0.1f, 0xFFC0C0C0);
			if(mTextView != null)itemsSencondPaint.setTypeface(mTextView.getTypeface());
		}
		if (valuePaint == null) {
			...
			if(mTextView != null)valuePaint.setTypeface(mTextView.getTypeface());
		}

		if (centerDrawable == null) {
			centerDrawable = getContext().getResources().getDrawable(
					R.drawable.doov_wheelview_center_bg);
		}
	...
	private String buildText(boolean useCurrentValue) {
		...
		for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
			if (useCurrentValue || /*i != currentItem*/(i<currentItem-1)||(i>currentItem+1)) {
	...
	private String buildSencondLineText(boolean useCurrentValue) {
		StringBuilder itemsText = new StringBuilder();
		int addItems =visibleItems / 2 + 1;
		for (int i = currentItem - addItems; i <= currentItem + addItems; i++) {
			if (useCurrentValue || ((i>=currentItem-1)&&(i<=currentItem+1)&&(i!=currentItem))) {
				String text = getTextItem(i);
				if (text != null) {
					itemsText.append(text);
				}
	...
	private void createLayouts(int widthItems, int widthLabel) {
		...
		if (!isScrollingPerformed
				&& (itemSencondLayout == null || itemSencondLayout.getWidth() > widthItems)) {
			itemSencondLayout = new StaticLayout(buildSencondLineText(isScrollingPerformed),
					itemsSencondPaint, widthItems,
					widthLabel > 0 ? Layout.Alignment.ALIGN_CENTER
							: Layout.Alignment.ALIGN_CENTER, 1,
					ADDITIONAL_ITEM_HEIGHT, false);
			...
			itemSencondLayout.increaseWidthTo(widthItems);
		}
		...
		if (!isScrollingPerformed
			...
			String text = getAdapter() != null ? getAdapter().getItem(
					currentItem) : null;
			valuePaintWidth = valuePaint.measureText(text);
		...
		if (widthLabel > 0) {
			...
			if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
				lablePaintWidth = valuePaint.measureText(label);
...
//		drawShadows(canvas);
	...
	private void drawValue(Canvas canvas) {
		...
		if (valueLayout != null) {
			...
			canvas.translate(0, bounds.top + scrollingOffset+5);
		...
		if (labelLayout != null) {
			...
			float dx = isEnglishLocale() ? itemsWidth - LABEL_OFFSET * 3 : itemsWidth-labelWidth + LABEL_OFFSET * 2;
			canvas.translate(dx , bounds.top);
	...
	private static boolean isEnglishLocale() {
		...
		// draw current value
	private void drawSencondLine(Canvas canvas) {
		itemsSencondPaint.setColor(ITEMS_TEXT_COLOR_SENCOND);
		itemsSencondPaint.drawableState = getDrawableState();
		int top = itemsLayout.getLineTop(1);
		itemsSencondPaint.setTextSize(TEXT_SIZE);
		
		if(itemSencondLayout!= null){
		canvas.translate(0, -top + scrollingOffset);
		itemSencondLayout.draw(canvas);
			canvas.restore();
		}
	}
